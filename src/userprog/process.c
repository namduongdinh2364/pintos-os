...
{
  struct list_elem *e;
  struct child *p_child = NULL;

  /* Find the child process corresponds to child_tid */
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  t->sup_pg_table = vm_sup_pg_table_init();

  /* Open executable file. */
  fn_cp = malloc (strlen(file_name) + 1);
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      /** Add inf into sup table */
      vm_sup_pg_table_set_page(upage, writable);
      file = file_reopen(file);
      vm_sup_pg_table_push_to_FILE(upage, file, ofs, page_read_bytes, page_zero_bytes, thread_current());
      ofs+= PGSIZE;
